...
export class MicrosoftCalendarProvider implements IntegrationProvider {
  ...
  async disconnect(integrationId: string): Promise<void> {
    try {
      ...
      const { error } = await supabase.functions.invoke("revoke-microsoft-tokens", {
        method: "POST",
      ...
      await IntegrationService.logIntegrationActivity(
        ...
        "disconnect",
        "success",
        "Microsoft Calendar disconnected successfully"
      );
    } catch (error: any) {
  ...
  async refresh(integrationId: string): Promise<void> {
    try {
      const { error } = await supabase.functions.invoke("microsoft-calendar-list");
      if (error) throw error;
      ...
      await IntegrationService.logIntegrationActivity(
        ...
        "refresh",
        "success",
        "Microsoft Calendar tokens refreshed successfully"
      );
    } catch (error: any) {
      console.error("Error refreshing Microsoft Calendar:", error);
      await IntegrationService.logIntegrationActivity(
        integrationId,
        "refresh",
        "failure",
        "Failed to refresh Microsoft Calendar tokens",
        { error: error.message }
      );
      throw error;
    }
    ...
    try {
      ...
      if (error) {
        await IntegrationService.logIntegrationActivity(
          ...
          "test",
          "failure",
          "Microsoft Calendar connection test failed",
          { error: error.message }
        );
        return false;
      }
      ...
      await IntegrationService.logIntegrationActivity(
        ...
        "test",
        "success",
        "Microsoft Calendar connection test successful"
      );
      return true;
    } catch (error: any) {
      console.error("Error testing Microsoft Calendar connection:", error);
      await IntegrationService.logIntegrationActivity(
        integrationId,
        "test",
        "failure",
  ...
  getConfigurationSchema(): Record<string, any> {
    return {
      calendar_id: {
        type: "string",
        label: "Calendar ID",
        description: "The Outlook calendar ID to use for bookings",
      },
      default_duration: {
        type: "number",
